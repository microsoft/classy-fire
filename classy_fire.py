...
class LLMClassifier:
    """
    A generic LLM based classifier that can be used to classify a string into
    a set of classes.

    Usage example:
        classifier = LLMClassifier(
            ["Grapes", "Bananas", "Stars"],
            gpt_api_config
        )
    ...
    system_prompt_base: str = (
        "You are an AI assistant. Select for the following human input the "
        "most related item from the following set of <options> and leveraging "
        "information given in <context>. Respond with the number "
    ...
    def __init__(
        ...
        class_id_tokens = [
            tokenizer.encode(str(i)) for i in range(len(class_names))
        ]
        assert all(
            len(token_list) == 1 for token_list in class_id_tokens
        ), f"A class id number did not map to a single token, maybe too many \
        classes in list: {len(class_names)}"
    ...
    def __call__(self, input_string: str) -> Tuple[str, int]:
        """
        The function takes a string and classifies it to a class name and
        class id.
        """
        response = self._llm_chat(
            [
                self.system_message,
                HumanMessage(
                    content=self.user_prompt_base.format(
